...
ThemeList.prototype.onSetTheme = function (paramTheme) {
	this.themes.forEach((theme) => {
		if (ThemeList.identical(theme, paramTheme)) {
			theme.elem.css('background-color', Theme.getRandomColor(theme)).addClass('active');
		} else {
...
ThemeList.prototype.read = function () {
	let self = this;
	this.cookie = ThemeList.parse( decodeURIComponent( Tools.getCookie( Config.cookieName))) || [];
	this.themes = ThemeList.validate( this.cookie);
	if (this.themes.length === 0) {
		this.config.themes.forEach((theme) => this.themes.push(_.assign({}, theme)));
	} else {
		setTimeout(function() {
...
ThemeList.prototype.onSaveTheme = function (newTheme) {
	if (Theme.validate(newTheme)) {
		...
		const identical = this.themes.some((theme) => ThemeList.identical(theme, newTheme));
		if (!identical) {
...
ThemeList.prototype.removeTheme = function (removeTheme) {
	...
	this.themes = this.themes.filter((theme) => !Theme.getValidKeys().every((key) => removeTheme[key] === theme[key]));
	this.save()
		.paint();
	return this;
}
...
ThemeList.prototype.paint = function () {
	...
	this.themes.map((theme) => {
		...
		keys.forEach((key) => {
			theme.elem.append($('<div>', {
				class: 'color-item',
			}).css('background-color', theme[key]));
		});
		let control = $('<span><i class="fa fa-times-circle" aria-hidden="true"></i></span>', {
			id: 'theme-remove-' + slug,
		}).addClass('control').click((e) => {
			e.preventDefault();
	...
	this.elem.append($('<div class="credits"><a target="_blank" href="https://aumbadgah.com">aumbadgah</a></div>'));
